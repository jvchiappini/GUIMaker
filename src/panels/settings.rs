...
const C_SURFACE: [f32; 4] = [0.145, 0.145, 0.149, 1.0];
const C_TITLE_BAR: [f32; 4] = [0.18, 0.18, 0.184, 1.0];
const C_ACCENT: [f32; 4] = [0.0, 0.478, 0.800, 1.0];
const C_TEXT: [f32; 4] = [0.867, 0.867, 0.867, 1.0];
const C_TEXT_MUTED: [f32; 4] = [0.60, 0.60, 0.60, 1.0];
const C_INPUT_BG: [f32; 4] = [0.098, 0.098, 0.098, 1.0];
const C_INPUT_BORDER: [f32; 4] = [0.267, 0.267, 0.267, 1.0];
const C_HOVER_CLOSE: [f32; 4] = [0.80, 0.22, 0.22, 1.0];
...
pub fn draw(dc: &mut DrawContext<'_, '_>, preview_w: f32, preview_h: f32) {
    ...
    dc.gui
        .rect(dx, dy + RADIUS, MODAL_W, TITLE_H - RADIUS, C_TITLE_BAR);
    ...
    let h_row_y = w_row_y + 52.0;
    dc.gui
        .draw_text(dc.font, "Alto (px)", [label_x, h_row_y + 8.0], 12.0, C_TEXT);
    draw_input(
        dc,
        input_x,
        h_row_y + 2.0,
    ...
    dc.gui.draw_text(
        dc.font,
        "Clic en valor para +10  ·  Arrastra los handles para redimensionar",
        [dx + pad, h_row_y + 56.0],
        10.0,
        C_TEXT_MUTED,
    );
...
fn draw_input(dc: &mut DrawContext<'_, '_>, x: f32, y: f32, w: f32, h: f32, value: &str) {
    // Borde
    dc.gui
        .rect_r(x - 1.0, y - 1.0, w + 2.0, h + 2.0, C_INPUT_BORDER, 4.0);
    // Fondo
    dc.gui.rect_r(x, y, w, h, C_INPUT_BG, 4.0);
    // Valor
    dc.gui
        .draw_text(dc.font, value, [x + 8.0, y + 7.0], 12.0, C_TEXT);
}
